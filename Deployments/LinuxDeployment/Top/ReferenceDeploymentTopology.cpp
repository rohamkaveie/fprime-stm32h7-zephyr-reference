...
void configureTopology(const TopologyState& state) {
    ...
    // Linux GPIO driver does not require special open() call like Zephyr
    // GPIO functionality is available through the LinuxGpioDriver component
}
...
void setupTopology(const TopologyState& state) {
    ...
    // Configure communication driver based on state
    // For Linux: use TCP/IP socket or serial device
    if (state.hostname != nullptr && state.port != 0) {
        // TCP/IP mode - Svc.ComQueue handles this internally
        Fw::Logger::log("[Linux] Using TCP/IP: %s:%d\n", state.hostname, state.port);
    } else if (state.uartDevice != nullptr) {
        // Serial UART mode
        Fw::Logger::log("[Linux] Using UART device: %s at %u baud\n", state.uartDevice, state.baudRate);
    } else {
        Fw::Logger::log("[Linux] No communication interface specified\n");
    }
